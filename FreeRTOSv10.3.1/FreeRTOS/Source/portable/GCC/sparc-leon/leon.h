...
extern "C" {
  ...
  #define B2BSTORE_FIX_STR "nop\n\t"
#else
  #define B2BSTORE_FIX
  #define B2BSTORE_FIX_STR
#endif
...
unsigned int leonbare_leon23_storenocache(unsigned int addr,unsigned int value);
unsigned int leonbare_leon23_storenocache16(unsigned int addr,unsigned int value);
...
extern __inline__ void sparc_leon2_dcache_flush(void) {
	__asm__ __volatile__("sta %%g0, [%%g0] %0\n\t": :
			     "i" (ASI_LEON2_IFLUSH) : "memory");
	__asm__ __volatile__("sta %%g0, [%%g0] %0\n\t": :
			     "i" (ASI_LEON2_DFLUSH) : "memory");
};
